...
buttons_names = [first_col[0], second_col[0], third_col[0], forth_col[0]]
...
def window1():
    ...
    section1 = [[sg.Slider(range=(1, 1000), orientation='h', size=(34, 20), default_value=0)],
                [sg.Button(f'{first_col[0]}', font='{}'.format(fonts[0])),
                 sg.Button(f'{second_col[0]}', font='{}'.format(fonts[0])),
    ...
    tab1_col = [[sg.Checkbox(f'({i})', enable_events=True, font='consolas 10', key=f'position{i}')
                  for i in second_col]]
    tab1_layout = [sg.Column(tab1_col, size=(200,200), scrollable=True)]
    ...
    window = sg.FlexForm('Main Window', layout, finalize=True).Layout(tab1_layout)
